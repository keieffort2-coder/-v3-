...
function shouldSendImageSize(model) {
  ...
  return normalized !== "gemini-3-pro-image-preview" && normalized !== "gpt-image-2";
}
...
async function runImageGeneration(node) {
  ...
  const requestedSize = shouldSendImageSize(selectedModel) ? await resolveGenerationSize(prompt, referencePlan) : "";
