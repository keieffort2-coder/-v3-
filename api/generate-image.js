...
function buildImageEditPrompt(
  ...
  if (hasEditBase) {
    if (hasStructureReference && mode === "structureStyle") {
      return [
        ...
        "- The first input image is the SCENE STRUCTURE reference. It controls composition, camera, perspective, layout, object positions, and canvas ratio.",
        `- The next ${Math.max(0, styleCount)} input image(s) are STYLE references. Use them strongly for palette, color grading, light temperature, contrast, material language, atmosphere, and render finish.`,
        "- Keep the structure reference spatial layout stable while applying the style reference's color tone and finish.",
        "- Do not use style references to change composition, camera, architecture, crop, or object placement.",
        "- Do not use the structure reference's original color palette when a style reference is provided; preserve its layout only.",
      ].join("\n");
    }
    return [
      ...baseRules,
  ...
  if (mode === "structureStyle") {
    ...
    return [
      ...
      `- The next ${Math.max(0, styleCount)} input image(s) are STYLE references only. Use them strongly for color palette, color temperature, saturation, contrast, value grouping, material language, lighting mood, atmosphere, texture, and render-finish samples.`,
      ...
      "If a style reference is provided, the final color tone and material finish should visibly match the style reference more than the raw structure reference.",
